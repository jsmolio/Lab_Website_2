...
function loadStatsPage(){
	...
	for (var i = 2; i < table.rows.length; i++) {
		...
		var op = parseInt(table.rows[i].cells[3].innerHTML);
		if (home > op) {
...
function loadPlayersPage(){
	...
	for (var i = 0; i < players.length; i++) {
		...
		newElement.href = "#";
		newElement.onclick = function(){switchPlayers(event)};
		newElement.class = "dropdown-item";
...
function switchPlayers(event){
	var playerNum = players.findIndex(i => i.name === event.target.innerHTML);
